...
namespace edm {
  ...
  void RootInputFileSequence::initTheFile(
    ...
    try {
      ...
      //std::cout << "\n Done initiating request to open file" ;
      //HERE
      //std::cout << "\n fileNames()" << fileName() ;
      //std::cout << "\n rawPnf " << fileIter_->rawPfn() ;
      //std::cout << "\n rawLnf " << fileIter_->rawLfn() ;
      ...
      if (!skipBadFiles) {
        if (hasFallbackUrl) {
          ...
          //throw ex;
          std::cout << "\n There is no fallback. Move to other data catalog" << std::endl ;
        }
      }
    }
    ...
    if (!filePtr && (hasFallbackUrl)) {
      try {
        ...
        //std::cout << "\n Done fallback request to open file" ;
        //HERE
        //std::cout << "\n fileNames()" << fileName() ;
        //std::cout << "\n rawPnf " << fileIter_->rawPfn() ;
        //std::cout << "\n rawLnf " << fileIter_->rawLfn() ;
        ...
        if (!skipBadFiles) {
          ...
          //}
          //throw ex;
          std::cout << "\n Attempt to open file using fall back data catalog fails. Move to other data catalog" << std::endl ;
        }
      }
    }
    ...
    //HERE
    if (!filePtr) {
      for (unsigned int iTmp = 0 ; iTmp < catalog_.n_otherFileLocator() ; ++iTmp) {
        try {
          std::unique_ptr<char[]> name(gSystem->ExpandPathName(catalog_.getFileName_otherFileLocators(fileIter_->rawLfn(), iTmp).c_str()));
          filePtr.reset(new InputFile(name.get(), "  Request to file using other data catalog", inputType));
          //std::cout << "\n Done request to open file using other data catalog" ;
          break ;
        }
        catch(cms::Exception const& e) {
          continue ;
